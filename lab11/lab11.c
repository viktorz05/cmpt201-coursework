...
#define ssl_error()                                                            \
  ...
  size_t mlen = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  size_t slen = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  ...
  if ((EVP_DigestVerifyUpdate(mdctx, message, mlen)) != 1) {
    ssl_error();
    return -1;
  }
